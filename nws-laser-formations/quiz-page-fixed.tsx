...
export default function QuizPage({ params }: QuizPageProps) {
  ...
  const calculateScore = () => {
    ...
    quiz.questions.forEach((question: any, index: number) => {
      // Vérifier si correctAnswer existe, sinon utiliser correctAnswers[0]
      ...
      if (correctAnswer !== null && selectedAnswers[index] === correctAnswer) {
        correctCount++;
      }
    });
  ...
  const submitQuiz = async () => {
    ...
    const isPassed = calculatedScore >= (quiz?.passingScore || 70);
    ...
    try {
      ...
      if (isPassed) {
        ...
        toast.info(`Vous avez obtenu un score de ${calculatedScore}%. Score minimum requis: ${quiz?.passingScore || 70}%`);
      }
    } catch (error) {
              ...
              {quizResult && (
                <div className={`mb-6 p-4 rounded-md ${quizResult.passed ? 'bg-green-50 border border-green-200' : 'bg-yellow-50 border border-yellow-200'}`}>
                  ...
                  <p className="text-gray-600 mt-1">
                    Tentatives: {quizResult.attempts || 1}
                  </p>
                </div>
              )}
        ...
        <div className="max-w-3xl mx-auto px-4">
          <div className="bg-white rounded-lg shadow-sm overflow-hidden mb-8">
            ...
            <div className="p-6">
              ...
              {Array.isArray(quiz.questions) && quiz.questions.map((question: any, index: number) => {
                // Vérification que correctAnswer est défini
                const correctAnswer = question.correctAnswer !== undefined 
                  ? question.correctAnswer 
                  : (Array.isArray(question.correctAnswers) && question.correctAnswers.length > 0 
                      ? question.correctAnswers[0] 
        ...
        <div className="bg-white rounded-lg shadow-sm overflow-hidden mb-8">
            ...
            <div className="flex items-center mt-2">
              <div className="w-full bg-gray-200 rounded-full h-2 mr-4">
                <div 
                  ...
                  style={{ width: `${((currentQuestion + 1) / (Array.isArray(quiz.questions) ? quiz.questions.length : 1)) * 100}%` }}
                ></div>
              </div>
              <span className="text-sm text-gray-600 whitespace-nowrap">
                Question {currentQuestion + 1}/{Array.isArray(quiz.questions) ? quiz.questions.length : 1}
              </span>
            </div>
          </div>
          ...
          <div className="p-6">
                  ...
                  {Array.isArray(quiz.questions[currentQuestion].options) 
                    ...
                    : (
                      <div className="p-4 border rounded-lg bg-red-50">
                        <p className="text-red-600">Erreur: format d'options invalide</p>
                      </div>
                    )
                  }
                </div>
            ...
            <div className="flex justify-between">
              ...
              {Array.isArray(quiz.questions) && currentQuestion < quiz.questions.length - 1 ? (
                <button
